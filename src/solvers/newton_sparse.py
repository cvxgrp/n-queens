...
@dataclass
class SparseHessianAffineCvxProblem:
    __doc__ = """
    ...
    nabla_squared_f: Callable[[np.ndarray], scipy.sparse.dia_matrix]
...
def _newton_step(max_iters, tolerance, problem):
    ...
    nabla_squared_f = problem.nabla_squared_f
    ...
    def build_minres_LHS(splu_nabla_squared_f_x):
        def matvec(v):
            z1 = A.T @ v
            z2 = splu_nabla_squared_f_x.solve(z1)
            z3 = A @ z2
            return z3
    ...
    def compute_update_direction(pair):
        """
            ...
            nabla^2f(x).
        """
        nabla_squared_f_x = nabla_squared_f(pair.x).tocsc()
        splu_nabla_squared_f_x = scipy.sparse.linalg.splu(nabla_squared_f_x)

        inv_nabla_squared_f_r_dual = splu_nabla_squared_f_x.solve(pair.r_dual) 
        #LHS = A @ scipy.sparse.linalg.spsolve(nabla_squared_f_x, A.T)
        LHS = build_minres_LHS(splu_nabla_squared_f_x)
        ...
        dv = -neg_dv
        dx = -splu_nabla_squared_f_x.solve(pair.r_dual + A.T @ dv)
        return dx, dv
    ...
    def construct_x_from_bounds(lower_bound_f, upper_bound_f):
        initial_x = np.nan * np.ones(dim_variables)
